...
# ВСЁ РАБОТАЕТ: чаты, имя, темы, поиск, игры, счётчик, ГДЗ, МОДЕЛИ, МЫШЛЕНИЕ, МОНЕТКА, AI
# ============================================
...
import urllib.parse
import subprocess
import requests   # ← для бесплатной нейросети
...
def process_ai(text):
    ...
    # === ЕСЛИ ВЫБРАНА ВНЕШНЯЯ МОДЕЛЬ (OLLAMA) ===
    if current_ai_model != "myxa":
        try:
            prompt = f"Think step by step: {text}" if (deep_thinking and "deepseek" in current_ai_model) else text
            command = f'cmd /c "ollama run {current_ai_model} {prompt}"'
            ...
            if output:
                ...
                return "🤖 Модель не ответила. Проверь, скачана ли она через `ollama pull`."
        except subprocess.TimeoutExpired:
            return "⏰ Модель думает слишком долго. Попробуй позже."
        except Exception as e:
            return f"❌ Ошибка запуска: {str(e)[:80]}"
...
def open_help():
    ...
    help_text = (
        ...
        "📚 ШКОЛА: таблица 5\n\n"
        "🧠 МОДЕЛИ: кнопка «Модель» — выбор ИИ\n\n"
        "🤖 AI: кнопка «AI» — бесплатная нейросеть"
    )
...
def choose_model():
    """Окно выбора ИИ-модели с выпадающим списком"""
    model_win = tk.Toplevel(root)
    model_win.title("🤖 Выбор модели")
    model_win.geometry("350x300")
    model_win.resizable(False, False)
    
    tk.Label(model_win, text="ВЫБЕРИ МОЗГИ", font=("Arial", 14, "bold")).pack(pady=15)
    
    default_models = ["myxa", "phi3", "llama3.2", "mistral", "deepseek-r1:7b"]
    
    try:
        result = subprocess.run(["ollama", "list"], capture_output=True, text=True, shell=True, encoding='utf-8')
        ...
        for m in default_models:
            if m not in models and m != "myxa":
                models.append(m)
    except:
        models = default_models

    if "myxa" not in models:
        models.insert(0, "myxa")
    
    tk.Label(model_win, text="Выбери модель:", font=("Arial", 11)).pack(pady=5)
    
    selected_model = tk.StringVar(value=current_ai_model)
    dropdown = tk.OptionMenu(model_win, selected_model, *models)
    dropdown.config(width=20, font=("Arial", 11))
    dropdown.pack(pady=10)
    
    def apply_model():
        global current_ai_model, deep_thinking
        current_ai_model = selected_model.get()
        if current_ai_model != "deepseek-r1:7b":
            deep_thinking = False
        ...
        display_name = model_names.get(current_ai_model, current_ai_model)
        add_message("MYXA AI", f"✅ Модель изменена: {display_name}")
        model_win.destroy()
    
    tk.Button(model_win, text="✅ Применить", command=apply_model,
              width=15, bg="#4CAF50", fg="white").pack(pady=10)
    
    tk.Label(model_win, text=f"Текущая: {current_ai_model}",
             font=("Arial", 10, "italic")).pack(pady=5)
    tk.Label(model_win, text=f"🧠 Мышление: {'ВКЛ' if deep_thinking else 'ВЫКЛ'}",
             font=("Arial", 10, "italic")).pack(pady=5)
    
    tk.Button(model_win, text="🔄 Обновить список",
              command=lambda: [model_win.destroy(), choose_model()]).pack(pady=5)
    tk.Button(model_win, text="❌ Закрыть", command=model_win.destroy).pack(pady=5)

# ============================================
#         КНОПКА AI (БЕСПЛАТНАЯ НЕЙРОСЕТЬ)
# ============================================
def ask_huggingface(prompt):
    """Бесплатная нейросеть через Hugging Face"""
    try:
...
def ai_thread(prompt):
    response = ask_huggingface(prompt)
    add_message("🤖 AI", response)

def ask_ai():
    prompt = simpledialog.askstring("🤖 Нейросеть", "Введи запрос:")
    if prompt and prompt.strip():
        add_message("Ты", prompt)
        threading.Thread(target=ai_thread, args=(prompt,)).start()
...
make_button("Модель", choose_model)
make_button("🧠 Мышление", toggle_deep_thinking)
make_button("🤖 AI", ask_ai)   # ← НОВАЯ КНОПКА
