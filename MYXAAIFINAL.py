...
# ВСЁ РАБОТАЕТ: чаты, имя, темы, поиск, игры, счётчик, ГДЗ
# ============================================
...
def process_ai(text):
    global user_name, message_count, secret_number
    ...
    if any(w in clean for w in ["что я говорил", "что я сказал", "помнишь", "напомни"]):
        if message_history:
            # Ищем последнее сообщение пользователя (чётные индексы, если сохраняем парами)
            last_user_msg = ""
...
# === НОВЫЕ КОМАНДЫ КОНТЕКСТА ===
    ...
    # Базовые ответы (если имя есть)
    if "кто ты" in clean or "что ты" in clean:
        return build_reply(f"Я MYXA AI — твой оффлайн-помощник. Сообщений: {message_count} 😎", emo_reply)
    if clean == "спасибо":
