...
# ============================================
# ----------- БЕСПЛАТНАЯ НЕЙРОСЕТЬ (DIALOGPT) -
# ============================================

def ask_free_ai(prompt):
    """Бесплатная нейросеть Flan-T5 (Google) — точно работает"""
    import urllib.request
    import json
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=45) as response:
            ...
            if isinstance(result, list) and len(result) > 0:
                return result[0].get('generated_text', '🤖 Не понял вопрос')
            else:
                return str(result)[:200]
    except Exception as e:
        return f"❌ Ошибка сети. Проверь интернет.\n{str(e)[:50]}"
...
def open_help():
    ...
    help_text = (
        ...
        "📚 ШКОЛА: таблица 5\n\n"
        "🤖 Нейросеть: кнопка «🧠 Нейро» — бесплатная DialoGPT"
    )
...
def switch_to_neural():
    ...
    add_message("MYXA AI", "🧠 Режим нейросети (DialoGPT) активирован. Все сообщения будут обрабатываться ИИ.")
